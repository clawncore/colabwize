...
class FeedbackService {
  ...
  async createFeedback(feedbackData: {
    ...
    if (feedbackData.type === "feature_request") {
      return this.submitFeatureRequest({
  ...
  // Submit a feature request (public/anonymous supported)
  async submitFeatureRequest(data: {
    title: string;
    description: string;
    category?: string;
    priority?: string;
  }): Promise<{ success: boolean; message: string; featureRequestId: string }> {
    const response = await apiClient.post("/api/feature-request/simple", data);
    return response;
  }
