  ...
  (
    ...
    return (
      <div className="space-y-2 mt-4">
        {label && (
          <label className="text-sm font-medium text-[#C7CBD6] dark:text-[#C7CBD6] block">
            {label}
          </label>
        )}
        <div className="relative">
          {leftIcon && (
            <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#8B90A1]">
              {leftIcon}
            </div>
          )}
          <input
            type={inputType}
            className={cn(
              "flex h-12 w-full rounded-xl border border-gray-300 px-4 py-2 text-sm transition-all duration-200",
