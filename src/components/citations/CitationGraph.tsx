...
export const CitationGraph: React.FC<CitationGraphProps> = ({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (
        project.outline &&
        Array.isArray(project.outline) &&
        project.outline.length > 0
      ) {
        const distinctL1 = project.outline
          .filter((i: any) => i.level === 1)
          .slice(0, 5);
        topics = distinctL1.map((l1: any) => {
          return {
            label: l1.title,
            subtopics: ["Key Concept", "Analysis", "Evidence"],
          };
        });
      }
      ...
      if (topics.length === 0) {
        ...
        if (title.includes("dna") || title.includes("genomics")) {
          topics = [
            ...
            {
              label: "Sequencing Generations",
              subtopics: ["Sanger", "Next-Gen (NGS)", "Nanopore"],
            },
            {
              label: "NGS File Formats",
              subtopics: ["FASTQ", "BAM/SAM", "BED"],
            },
            {
              label: "Quality Control",
              subtopics: ["FastQC Metrics", "Trimming", "Filtering"],
            },
          ];
          ...
          topics = [
            {
              ...
              subtopics: ["Blood-borne", "Vertical", "Sexual"],
            },
            {
              label: "Prevention Strategies",
              subtopics: ["PrEP / PEP", "Vaccine Research", "Education"],
            },
            {
              label: "Treatment Protocols",
              subtopics: ["ART Adherence", "Viral Load", "Drug Resistance"],
            },
            {
              label: "Socio-economic Impact",
              subtopics: ["Healthcare Costs", "Stigma", "Labor Force"],
            },
          ];
        } else {
          topics = [
            {
              ...
              subtopics: ["Historical Context", "Major Theories", "Gaps"],
            },
            {
              label: "Methodology",
              subtopics: ["Research Design", "Data Collection", "Participants"],
            },
            {
              label: "Data Analysis",
              subtopics: ["Statistical Models", "Qualitative Coding"],
            },
            {
              label: "Discussion",
              subtopics: ["Implications", "Limitations", "Future Work"],
            },
          ];
        }
      }
  ...
  const renderEdges = () => {
    ...
    return nodes.flatMap((sourceNode) => {
      if (!sourceNode.childrenIds || sourceNode.childrenIds.length === 0)
        return [];
    ...
    <div className="h-full w-full flex flex-col bg-white overflow-hidden relative">
      ...
      <div className="absolute top-4 left-4 z-20 flex items-center gap-3 bg-white/95 backdrop-blur-md p-2 pl-3 rounded-lg border border-gray-100 shadow-md max-w-[calc(100%-32px)]">
        ...
        <div className="pointer-events-none">
          <h1 className="text-lg font-bold text-gray-900 mb-0.5 whitespace-nowrap overflow-hidden text-ellipsis">
            Visual Insight Map
          </h1>
          <p className="text-[10px] text-gray-500 leading-tight">
            Topic relationships. Click to explore.
          </p>
        </div>
      </div>
        ...
        <div
          ...
          {nodes.map((node) => (
            <div
              ...
              <span
                className={`text-sm font-semibold truncate ${node.type === "root" ? "text-base" : ""}`}>
                {node.label}
              </span>
