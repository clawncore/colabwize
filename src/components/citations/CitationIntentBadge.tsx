...
export const IntentLegend: React.FC<IntentLegendProps> = ({
  compact = false,
}) => {
  ...
  if (compact) {
    return (
      <div className="flex flex-wrap gap-2">
        {intents.map((intent) => (
          <CitationIntentBadge key={intent} intent={intent} />
        ))}
      </div>
    );
  }
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="flex items-center gap-2">
          <CitationIntentBadge intent="supporting" showLabel={false} />
          <span className="text-xs text-gray-600">Supports claim</span>
        </div>
        <div className="flex items-center gap-2">
          <CitationIntentBadge intent="contrasting" showLabel={false} />
          <span className="text-xs text-gray-600">Opposes claim</span>
        </div>
        <div className="flex items-center gap-2">
          <CitationIntentBadge intent="background" showLabel={false} />
          <span className="text-xs text-gray-600">Background info</span>
        </div>
        <div className="flex items-center gap-2">
          <CitationIntentBadge intent="methodology" showLabel={false} />
