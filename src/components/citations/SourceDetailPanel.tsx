...
interface SourceDetailPanelProps {
  ...
  isPremium?: boolean; // Added for entitlement check
}
...
export const SourceDetailPanel: React.FC<SourceDetailPanelProps> = ({
  ...
  const handleAnalyze = async () => {
    ...
    if (!source.abstract) {
      alert(
        "No abstract available for this source. Please use a source with an abstract.",
      );
      return;
    }
    ...
    try {
      const { CitationService } =
        await import("../../services/citationService");
      const updated = await CitationService.analyzeCitation(
        projectId,
        source.id,
      );
      if (updated) {
  ...
  const handleToggleTheme = async (theme: string) => {
    ...
    try {
      ...
      const { CitationService } =
        await import("../../services/citationService");
      await CitationService.updateCitationThemes(projectId, source.id, {
        themes: newThemes,
      });
  ...
  const handleAnalyzeConsensus = async () => {
    ...
    if (!source.abstract) {
      alert(
        "Cannot analyze: This paper doesn't have an abstract. Please try adding it from a search result that includes abstracts.",
      );
      return;
    }
    ...
    try {
      const response = await apiClient.post(
        `/api/citations/${projectId}/consensus`,
        {
      ...
      <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
        ...
        <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-xl relative overflow-hidden">
          ...
          <p className="text-xs text-gray-500 mb-3 leading-relaxed">
            Assess whether this paper supports, opposes, or remains neutral
            regarding a specific research claim.
          </p>
          ...
          <div className="mb-3">
            <input
              ...
              disabled={isAnalyzingConsensus || !isPremium}
            />
          </div>
          ...
          <button
            ...
            disabled={
              !consensusClaim.trim() || isAnalyzingConsensus || !isPremium
            }
            ...
            <div className="mt-4 p-3 bg-white border border-gray-200 rounded-lg">
              ...
              {consensusResult.supporting.length > 0 && (
                <div className="text-xs text-emerald-700 mb-1">
                  ✓ <strong>{consensusResult.supporting.length}</strong>{" "}
                  supporting
                </div>
              )}
