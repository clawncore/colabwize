...
import {
  ...
  WritingPatternData,
} from "../../services/behavioralTrackingService";
...
const BehavioralTracker: React.FC<BehavioralTrackerProps> = ({
  ...
  useEffect(() => {
    ...
    const handleTextChange = () => {
      ...
      const newWordCount = content.trim()
    ...
    try {
      ...
      // View not available yet — skip event listener setup
      return;
    }
    ...
    const interval = setInterval(() => {
      if (projectId && userId) {
        const behaviorData: Partial<WritingPatternData> = {
          ...
          errorRate:
            deletionCountsRef.current / (typingSpeedsRef.current.length || 1),
          averageThinkTime:
            pauseDurationsRef.current.reduce((sum, val) => sum + val, 0) /
            (pauseDurationsRef.current.length || 1),
        };
