...
export const GrammarBubbleMenu = ({ editor }: GrammarBubbleMenuProps) => {
  ...
  useEffect(() => {
    const updatePosition = () => {
      ...
      const { $from } = selection;
      const grammarMark = $from
        .marks()
        .find((m) => m.type.name === "grammar-error");
  ...
  const handleFix = () => {
    ...
    editor
      .chain()
      .focus()
      .extendMarkRange("grammar-error")
      .insertContent(activeError.suggestion)
      .unsetMark("grammar-error")
      .run();
        ...
        <div className="flex items-center gap-2">
          <span
            className={`text-xs font-bold px-2 py-0.5 rounded-full uppercase ${
              activeError.type === "spelling"
                ? "bg-red-100 text-red-700"
                : activeError.type === "grammar"
