 ...
 */
export function findBibliographySection(
  text: string,
...
export function parseBibliographyEntries(text: string): BibliographyItem[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const isNewEntry =
      /^[A-Z]/.test(line) || /^\d+\./.test(line) || /^\[\d+\]/.test(line);
    ...
    try {
      ...
      editor.state.doc.descendants((node, pos) => {
        if (
          node.isBlock &&
          node.textContent &&
          node.textContent.includes(entry.text)
        ) {
      ...
      if (targetPos !== null && targetNode) {
        const bibNode = editor.state.schema.nodes.bibliographyEntry.create(
          {
            ...
            url:
              registryEntry.url || registryEntry.doi
                ? `https://doi.org/${registryEntry.doi}`
                : null,
            doi: entry.doi,
            refText: entry.text,
          },
        ...
        tr.replaceWith(targetPos, targetPos + targetNode.nodeSize, bibNode);
        modificationsMade = true;
        console.log(
          `✅ Normalized bibliography entry: ${entry.authors[0] || "Unknown Author"}`,
        );
      }
    } catch (error) {
