...
export const PDFAnnotator: React.FC<PDFAnnotatorProps> = ({
  ...
  const [activeTool, setActiveTool] = useState<"view" | "highlight" | "note">(
    "view",
  );
  ...
  const handleTextSelection = () => {
    ...
    const pageElement = (range.commonAncestorContainer as HTMLElement).closest(
      ".react-pdf__Page",
    );
        ...
        <div className="flex items-center gap-2 bg-gray-50 p-1.5 rounded-xl border border-gray-200 shadow-sm">
          ...
          <div className="flex items-center gap-1.5 ml-2 pl-3 border-l border-gray-200">
            {[
              ...
              {
                name: "Green",
                value: "rgba(34, 197, 94, 0.4)",
                class: "bg-green-400",
              },
              {
                name: "Cyan",
                value: "rgba(6, 182, 212, 0.4)",
                class: "bg-cyan-400",
              },
            ].map((c) => (
              <button
        ...
        <div className="w-80 bg-gray-50 border-r border-gray-100 flex flex-col z-10 shadow-sm">
          <div className="p-5 border-b border-gray-100 bg-white/50">
            <h2 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">
              Research Annotations ({annotations.length})
            </h2>
          </div>
            ...
            {annotations.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center opacity-50">
                ...
                <p className="text-xs text-gray-500">
                  ...
                  <br />
                  Use the tools to start annotating.
                </p>
              </div>
            ) : (
              annotations.map((ann, idx) => (
                <div
                  key={idx}
                  className="p-4 bg-white rounded-xl border border-gray-100 hover:border-[#6366F1] hover:shadow-md transition-all group cursor-pointer">
                  <div className="flex items-center justify-between mb-3">
                    <span
                      className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase ${ann.type === "highlight" ? "bg-yellow-100 text-yellow-700" : "bg-purple-100 text-purple-700"}`}>
                      {ann.type}
                    </span>
                    <button
                      onClick={() =>
                  ...
                  <p className="text-xs text-gray-700 line-clamp-4 italic leading-relaxed">
                    "{ann.content || "..."}"
                  </p>
                  <div className="mt-3 pt-3 border-t border-gray-50 text-[10px] text-gray-400 font-bold flex justify-between">
                    ...
                    <span className="text-[#6366F1] opacity-0 group-hover:opacity-100 transition-opacity">
                      Jump to Source
                    </span>
                  </div>
                </div>
            ...
            <Document
              ...
              loading={
                <div className="flex flex-col items-center justify-center p-20 bg-white rounded-2xl border border-gray-100 shadow-xl">
                  ...
                  <p className="text-xs text-gray-500 mt-1 uppercase tracking-widest font-bold opacity-70">
              ...
              error={
                <div className="p-10 bg-red-900/20 border border-red-500 rounded-lg text-center">
                  ...
                  <p className="text-xs text-red-300 mt-2">
                    Make sure the file is a valid scientific document.
                  </p>
                  <button
                    onClick={onClose}
