...
export default function CreateProjectModal({
  ...
  // Fetch workspace templates when workspace or type changes
  useEffect(() => {
    const fetchWorkspaceTemplates = async () => {
      if (selectedWorkspace && watchedFields.type) {
        try {
  ...
  }, [selectedWorkspace, watchedFields.type]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Create the project using documentService
