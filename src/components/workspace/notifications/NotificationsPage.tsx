...
const NotificationsPage: React.FC = () => {
    ...
    const fetchNotifications = useCallback(async (isLoadMore = false) => {
        ...
        try {
            ...
            const result = await NotificationService.getNotifications(PAGE_SIZE, currentOffset, filters);
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button
                        variant="outline"
                        size="sm"
                        onClick={() => navigate(`/dashboard/admin/${workspaceId}/notifications?tab=notifications`)}
                        className="text-slate-600 border-slate-200"
                    >
                        Settings
                    </Button>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <div className={`relative flex-1 min-w-[240px] ${isSearchVisible ? "block" : "hidden sm:block"}`}>
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                {loading ? (
                    ...
                    <div className="p-12 text-center">
                        ...
                        <h3 className="text-lg font-medium text-slate-900">Error Loading Notifications</h3>
                        <p className="text-slate-500 mt-2 max-w-xs mx-auto">{error}</p>
                        <Button variant="outline" className="mt-6" onClick={() => fetchNotifications(false)}>
                            Try Again
                        </Button>
                    </div>
                    ...
                    <div className="p-16 text-center">
                        ...
                        <p className="text-slate-500 mt-2 max-w-xs mx-auto">
                            We'll notify you when something important happens in your workspaces.
                        </p>
                    ...
                    <div className="divide-y divide-slate-100">
                        {notifications.map((notification) => (
                            <div
                                key={notification.id}
                                className={`group flex items-start p-4 gap-4 transition-colors hover:bg-slate-50 cursor-pointer ${!notification.read ? "bg-blue-50/50" : ""
                                    }`}
                                onClick={() => handleNotificationClick(notification)}
                            >
                                <div className={`mt-1 h-10 w-10 rounded-full flex items-center justify-center flex-shrink-0 ${!notification.read ? "bg-white shadow-sm ring-1 ring-slate-100" : "bg-slate-50"
                                    }`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between mb-0.5">
                                        <h4 className={`text-sm font-semibold truncate ${!notification.read ? "text-slate-900" : "text-slate-700 font-medium"
                                            }`}>
                                            {notification.title}
                                        </h4>
                                    ...
                                    <p className={`text-sm leading-relaxed line-clamp-2 ${!notification.read ? "text-slate-600" : "text-slate-500"
                                        }`}>
                                    ...
                                    <div className="flex items-center mt-2.5 space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleSnooze(notification.id); }}
                                ...
                                <div className="flex flex-col items-end gap-2">
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="bg-white">
                                            ...
                                            <DropdownMenuItem className="text-red-600" onClick={() => handleDismiss(notification.id)}>
