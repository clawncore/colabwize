    ...
    const fetchOverview = async () => {
        ...
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL || ""}/api/workspaces/${workspaceId}/overview`,
                {
                    headers: {
        ...
        <div className="p-8 w-full mx-auto max-w-7xl">
                ...
                <div className="flex gap-3 items-center">
                    ...
                    {canEdit && (
                        <Button
                            ...
                            onClick={() => navigate(`/dashboard/workspace/${workspaceId}/documents?tab=create`)}
                        >
                ...
                <Card className="shadow-sm border-slate-200">
                    <CardContent className="p-6 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-bold text-slate-900">{data.stats.projects.active}</h3>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent className="p-6 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-bold text-slate-900">
                                {data.stats.tasks.done} <span className="text-sm font-normal text-slate-400">/ {data.stats.tasks.total}</span>
                            </h3>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="shadow-sm border-slate-200">
                    <CardContent className="p-6 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-bold text-slate-900">{data.stats.tasks.todo + data.stats.tasks.in_progress}</h3>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="shadow-sm border-slate-200">
                    <CardContent className="p-6 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-bold text-slate-900">{data.stats.members}</h3>
                        </div>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <Card className="shadow-sm border-slate-200">
                        ...
                        <CardContent>
                            {data.recentProjects.length === 0 ? (
                                <div className="text-center py-8 text-slate-500 text-sm">No projects yet. Start by creating one!</div>
                            ) : (
                                    ...
                                    {data.recentProjects.map((project) => (
                                        ...
                                        >
                                            <div className="flex justify-between items-start mb-2">
                                                <div className={`p-2 rounded-lg ${project.status === 'completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'
                                                    }`}>
                                                    <Briefcase className="w-5 h-5" />
                                                </div>
                                                <Badge variant={project.status === 'active' ? 'default' : 'secondary'} className={
                                                    project.status === 'active' ? "bg-indigo-600 hover:bg-indigo-700" : ""
                                                }>
                                                    {project.status}
                                                </Badge>
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between text-xs text-slate-400">
                                                ...
                                                <div className="flex -space-x-2">
                                                    {project.members?.map((m: any) => (
                                                        <Avatar key={m.user.id} className="w-6 h-6 border-2 border-white">
                                                            <AvatarImage src={m.user.avatar_url} />
                                                            <AvatarFallback className="text-[10px] bg-slate-200">{m.user.full_name?.substring(0, 2).toUpperCase()}</AvatarFallback>
                                                        </Avatar>
                                                    ))}
                                                </div>
                                            </div>
                    ...
                    <Card className="shadow-sm border-slate-200">
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            <CardTitle className="text-lg font-semibold text-slate-900">My Priority Tasks</CardTitle>
                            <Button variant="ghost" size="sm" asChild className="text-slate-500 hover:text-indigo-600">
                                <Link to={`/dashboard/workspace/${workspaceId}/kanban?view=list`}>
                                    View All <ArrowRight className="w-4 h-4 ml-1" />
                                </Link>
                            </Button>
                        </CardHeader>
                            ...
                            {data.myTasks.length === 0 ? (
                                ...
                                <div className="space-y-3">
                                    {data.myTasks.map((task) => (
                                        <div key={task.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors">
                            ...
                            <ScrollArea className="h-[500px] px-6 pb-6">
                                <div className="space-y-6 relative border-l border-slate-100 ml-3">
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-center gap-2">
                                                    <Avatar className="w-5 h-5">
                                                        ...
                                                        <AvatarFallback className="text-[8px]">{activity.user?.full_name?.substring(0, 2)}</AvatarFallback>
                                                    </Avatar>
                                                    <span className="text-sm font-medium text-slate-900">{activity.user?.full_name}</span>
                                                </div>
                                                <p className="text-sm text-slate-600">
                                                    {activity.action.replace(/_/g, " ").toLowerCase()}
                                                    {activity.details?.target_name && <span className="font-medium"> {activity.details.target_name}</span>}
                                                </p>
                                    ...
                                    {data.recentActivity.length === 0 && (
                                        <p className="text-sm text-slate-400 pl-6 italic">No recent activity recorded.</p>
                                    )}
                                </div>
                            </ScrollArea>
                        </CardContent>
