...
export function TeamChat({
  ...
  const [editingMessage, setEditingMessage] = useState<TeamChatMessage | null>(
    null,
  );
  ...
  useEffect(() => {
    // Warm up the backend (Render fix)
      ...
      WorkspaceService.getWorkspace(effectiveWorkspaceId)
        .then((data) => {
          if (data) {
            ...
            // Fetch mention users via WorkspaceTaskService
            WorkspaceTaskService.getWorkspaceMembers(effectiveWorkspaceId)
              .then((membersData) => {
                const mapped = membersData.map((u: any) => ({
                  id: u.id,
  ...
  // Memoized handlers for useSupabaseChat to prevent unstable subscription
  ...
  const handleMessageUpdated = useCallback((updatedMsg: TeamChatMessage) => {
    setMessages((prev) =>
  ...
  const {
    ...
    workspaceId:
      activeChannel?.type === "workspace" ? activeChannel.id : undefined,
    projectId: activeChannel?.type === "project" ? activeChannel.id : undefined,
    currentUser: user,
  ...
  // Calculate online and typing sets for UI
  const onlineUsers = new Set(activeUsers.map((u) => u.id));
  const typingUsersNames: Record<string, string> = {};

  Object.keys(typingUsers).forEach((userId) => {
    if (typingUsers[userId] && userId !== user?.id) {
      const member = workspaceMembers.find((m) => m.user_id === userId);
      typingUsersNames[userId] = member?.user?.full_name || "Someone";
    }
  });
  ...
  }, [inputValue, activeChannel, user, sendTypingStatus]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const resolvedContent = inputValue.replace(
        /@\[([^\]]+)\](?!\()/g,
        (match, name) => {
      ...
      if (editingMessage) {
        ...
        await TeamChatService.editMessage(
          editingMessage.id,
          resolvedContent,
          contextId,
        );
        ...
        // Add to set of optimistic messages so Realtime hook ignores the DB event
        addOptimisticId(encryptedMsg.id);
        ...
        {activeChannel ? (
          <>
              ...
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500 flex items-center gap-1 truncate">
                    ...
                    <span className="truncate">E2E Encrypted</span>
                  </p>
                </div>
              </div>
                      ...
                      {hasMore && (
                        <div className="flex justify-center mb-4">
                          <Button
                            ...
                            className="text-xs bg-slate-50 text-slate-500 rounded-full hover:bg-slate-100 border-dashed">
                            {loadingMore ? (
                                  ...
                                  <div className="flex flex-col pr-14 relative min-h-[1.5rem]">
                                    ...
                                    <div className="absolute bottom-[-1px] right-0 flex items-center gap-1 opacity-60">
                                      {new Date(msg.updated_at).getTime() -
                                        ...
                                        1000 && (
                                        <span className="text-[9px] text-slate-400 italic mr-1">
                                          edited
                                        </span>
                                      )}
            ...
            <div className="p-2 bg-white flex flex-col border-t border-border">
              ...
              {editingMessage && (
                <div className="mx-2 mb-2 p-2 bg-white border-l-4 border-blue-500 rounded shadow-sm flex items-center justify-between text-xs animate-in slide-in-from-bottom-2">
                  <div className="flex flex-col truncate">
