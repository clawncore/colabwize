...
const ScoreBar = ({ label, score, weight }: any) => (
  <div className="text-sm">
    <div className="flex justify-between mb-1">
      ...
      <span
        className={`font-mono font-bold ${score > 80 ? "text-green-600" : score > 50 ? "text-amber-600" : "text-slate-400"}`}>
        {score}
      </span>
    </div>
...
const mapRuleToCodeAndColor = (
  ruleId: string,
): { code: string; color: string; label: string } => {
  const r = ruleId.toUpperCase();
  if (r.includes("VERIFICATION"))
    return { code: "VER", color: "red", label: "Source Verification" };
  if (r.includes("REF") || r.includes("MISMATCH") || r.includes("LIST"))
    return { code: "REF", color: "blue", label: "Reference Match" };
  if (r.includes("MIXED") || r.includes("CONSISTENCY"))
    return { code: "MIX", color: "amber", label: "Style Inconsistency" };
...
const filterViolationsByCode = (violations: any[], targetCode: string) => {
  ...
  return violations.filter(
    (v: any) => mapRuleToCodeAndColor(v.ruleId).code === targetCode,
  );
};
...
const AuditReportCard = ({
  ...
  <div
    className={`flex flex-col p-6 rounded-xl border transition-all bg-white shadow-sm hover:shadow-md ${expanded ? "ring-1 ring-offset-2" : ""} ${
      passed
        ? "border-gray-200"
        : code === "VER"
          ? "border-red-100"
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            {passed
              ? "All checks passed"
              : `${count} issue${count !== 1 ? "s" : ""} require attention`}
          </p>
        </div>
      </div>
      <div
    ...
    return (
      <div className="p-8 flex flex-col items-center justify-center h-full text-center">
        ...
        <p className="text-gray-500 mt-2 mb-6 max-w-md">
          Please run a citation audit from the document editor first to generate
          this report.
        </p>
                ...
                {auditResult.integrityIndex?.confidence && (
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full border ${
                      auditResult.integrityIndex.confidence === "HIGH"
              ...
              <div className="p-6 bg-slate-900 text-white flex-1 flex flex-col justify-between relative overflow-hidden">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm">
                    Based on style adherence, reference checking, and
                    verification.
                  </p>
                </div>
                ...
                <div className="mt-8 space-y-4">
                  <ScoreBar
                    ...
                    score={auditResult.integrityIndex.components.styleScore}
                    weight="30%"
                  />
                  <ScoreBar
                    label="Reference Check"
                    score={auditResult.integrityIndex.components.referenceScore}
                    weight="20%"
                  />
                  <ScoreBar
                    label="Existence Verification"
                    score={
                      auditResult.integrityIndex.components.verificationScore
                    }
                    weight="30%"
                  />
                  <ScoreBar
                    label="Semantic Alignment"
                    score={auditResult.integrityIndex.components.semanticScore}
                    weight="20%"
                  />
                </div>
              </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <AuditReportCard
              ...
              <div className="mt-6 pt-6 border-t border-gray-100 space-y-3">
                {filterViolationsByCode(auditResult?.violations, "REF").map(
                  (v: any, i: number) => (
                    <div
                      key={i}
                      className="p-4 border rounded-lg bg-blue-50/50 border-blue-100">
                ...
                {filterViolationsByCode(auditResult?.violations, "REF")
                  .length === 0 && (
                  <p className="text-gray-500 italic">
                    No reference mismatches found.
                  </p>
                )}
              </div>
            </AuditReportCard>
            ...
            <AuditReportCard
              ...
              <div className="mt-6 pt-6 border-t border-gray-100 space-y-3">
                {filterViolationsByCode(auditResult?.violations, "STY").map(
                  (v: any, i: number) => (
                    <div
                      key={i}
                      className="p-4 border rounded-lg bg-amber-50/50 border-amber-100">
                ...
                {filterViolationsByCode(auditResult?.violations, "STY")
                  .length === 0 && (
                  <p className="text-gray-500 italic">
