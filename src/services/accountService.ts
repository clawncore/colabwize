 ...
 */
export const getAccountUsage = async (): Promise<{
  projects: number;
  storage: number;
...
export const hasFeatureAccess = async (feature: string): Promise<boolean> => {
  try {
    ...
    const response = await apiClient.get(`/api/users/features/${feature}`);

    return response.hasAccess || false;
  } catch (error) {
    console.error(
      `Error checking feature access for feature ${feature}:`,
      error
    );
 ...
 */
/**
 * Export user account data as a ZIP file
 * @returns Promise resolving to the Blob containing the ZIP file
 */
export const exportAccountData = async (): Promise<Blob> => {
  try {
    // Initiate an account data export via API requesting ZIP format
    const response = await apiClient.download("/api/users/export-data", {
      format: "zip"
    });
