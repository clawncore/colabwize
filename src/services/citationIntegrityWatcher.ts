...
export class CitationIntegrityWatcher {
  ...
  static async processUpdates(
    editor: Editor,
    projectId: string,
    explicitUpdates?: Record<string, any>,
  ) {
    ...
    editor.state.doc.descendants((node, pos) => {
      if (node.type.name === "citation") {
        ...
        if (id) {
          ...
          if (
            node.attrs.status !== targetStatus ||
            node.attrs.url !== targetUrl ||
            node.attrs.sourceTitle !== targetTitle
          ) {
            nodesToUpdate.push({
              pos,
              id,
  ...
  private static async performAudit(
    ...
    const updates: Record<
      string,
      { status: CitationStatus; url?: string; sourceTitle?: string }
    > = {};
    ...
    keys.forEach((key) => {
      ...
      if (!entry) {
        ...
        updates[key] = {
          ...
          sourceTitle:
            entry.sourceTitle ||
            entry.csl_data?.title ||
            entry.raw_reference_text,
        };
      }
    });
