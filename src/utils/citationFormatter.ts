  ...
  if (type === "in-text") {
    ...
    switch (style) {
      ...
      case "Chicago":
        ...
        if (authors.length === 3)
          return `(${first}, ${authors[1]}, and ${authors[2]} ${year})`;
        return `(${first} et al. ${year})`;
...
export const generatePrecomputedCitations = (
  source: StoredCitation,
...
export const parseReferenceText = (text: string) => {
  // 1. IEEE numeric: [1] Author, Title, Year.
  const ieeeMatch = text.match(/^\[?(\d+)\]?[\.\s]/);

  // 2. Author-Year search
  const yearMatch =
    text.match(/\((\d{4}[a-z]?)\)/) || text.match(/\b(19|20)\d{2}\b/);
  ...
  if (ieeeMatch) {
    const afterMatch = text.substring(ieeeMatch[0].length).trim();
    primaryAuthor = afterMatch.split(/,|\.|&/)[0].trim();
  } else if (year > 0) {
    const idx = text.indexOf(year.toString());
    const preYearText = text.substring(0, idx).trim();
    primaryAuthor = preYearText.split(/,|\.|&/)[0].trim() || "Unknown";
  } else {
    primaryAuthor = text.split(/,|\.|&/)[0].trim() || "Unknown";
  }
  ...
  if (segments.length > 1) {
    // Assume first segment is authors, second is title or similar
